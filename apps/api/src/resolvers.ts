...
export const resolvers = {
  Query: {
    ...
    myAssets: async (_: any, args: { after?: string; first?: number; q?: string }, ctx: Context) => {
      const userId = requireAuth(ctx);
      ...
      return {
        ...
        pageInfo: {
          endCursor: nodes.length > 0 ? nodes[nodes.length - 1].created_at : null,
          hasNextPage,
        },
      };
    },
    ...
    getDownloadUrl: async (_: any, { assetId }: any, ctx: Context) => {
      ...
      if (asset.owner_id !== userId) {
        ...
        if (!sharedAccess) {
          throw new GraphQLError("Access denied: You are not authorized to download this asset.", {
      ...
      if (auditError) console.error("⚠️ Download audit log failed:", auditError.message);
  ...
  Mutation: {
    ...
    createUploadUrl: async (_: any, { filename, mime, size }: any, ctx: Context) => {
      ...
      if (!ALLOWED_MIMES.includes(mime)) {
        throw new GraphQLError(`Unsupported file type: ${mime}. Allowed: ${ALLOWED_MIMES.join(', ')}`, {
    ...
    deleteAsset: async (_: any, { assetId, version }: any, ctx: Context) => {
      ...
      if (storageError) console.error("⚠️ Storage deletion failed:", storageError);
